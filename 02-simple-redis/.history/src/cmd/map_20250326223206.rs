use crate::{ cmd::{ CommandError, Get, Set }, RespArray, RespFrame, RespNull };

use super::{ extract_args, validate_command, CommandExceutor, RESP_OK };
...
impl TryInto<RespArray> for Get {
    type Error = CommandError;

    fn try_into(self) -> Result<RespArray, Self::Error> {
        Ok(
            RespArray(
                vec![
                    RespFrame::BulkString("get".into()),
                    RespFrame::BulkString(self.key.into_bytes())
                ]
            )
        )
    }
